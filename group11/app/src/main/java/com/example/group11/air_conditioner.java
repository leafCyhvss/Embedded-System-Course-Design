...
import com.example.group11.air_conditioner.*;
...
public class air_conditioner extends Activity implements OnClickListener {
    ...
    public void onClick(View v) {
        ...
        switch (v.getId()) {
            case R.id.submit:
                ...
                break;
            case R.id.okoil:
                //判断输入电量是否合法（》100或《0要报错）
                String content1 = text2.getText().toString();
                int temp0=Integer.valueOf(content1);
                Air.oil=content1;
                //flag = false;
                if (temp0>100||temp0<0) {
                    Log.i("监听", content1);
                    Toast.makeText(getApplication(), "请输入正确电量百分比", Toast.LENGTH_LONG);
                } else {
    ...
    class MyThread extends Thread {
        public void run() {
            ...
            if(current>aim){
                isHot=false;//制冷
                current++;
                LedClass.IoctlLed(1, 1);//二灯亮
            }
            else{
                isHot=true;//制热
                current--;
                LedClass.IoctlLed(2, 1);//三灯亮
            }
...
//					通过Handler发送消息
                    ...
                    else if(oilint<=0){//没电四灯不亮
                        flag=false;
                        Looper.prepare();
                        Toast.makeText(air_conditioner.this,"小车电量不足",Toast.LENGTH_SHORT).show();
                        Looper.loop();
                    }
                    Thread.sleep(speed);
                } catch (InterruptedException e) {
